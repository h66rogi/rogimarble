...
runtime=['deploy/Caddyfile.production','deploy/postgres/init-roles.sh','deploy/systemd/rogimarble-app.service','deploy/systemd/rogimarble-slot@.service','deploy/systemd/rogimarble-secrets.service','deploy/systemd/rogimarble-update.service','deploy/systemd/rogimarble-update.timer','deploy/systemd/rogimarble-backup.service','deploy/systemd/rogimarble-backup.timer','tools/ops/release.py','tools/ops/deploy.sh','tools/ops/supervise.sh','tools/ops/supervise-slot.sh','tools/ops/prepare-secrets.sh','tools/ops/install-host.sh','tools/ops/fetch-release.py','tools/ops/production-status.py','tools/ops/backup-postgres.sh','tools/ops/fetch-runtime-secrets.py','tools/ops/upload-backup.py','tools/ops/load-registry-auth.py','tools/ops/prepare-collector-client.py']
...
with tempfile.TemporaryDirectory() as td:
 ...
 for path in sorted(stage.rglob('*')):
  ...
  elif path.is_file(): path.chmod(0o755 if path.read_bytes()[:2] == b'#!' else 0o644)
 with tarfile.open(out/'release-bundle.tar.gz','w:gz',format=tarfile.PAX_FORMAT) as tar:
  for p in sorted(stage.rglob('*')):
   executable = p.is_dir() or (p.is_file() and p.read_bytes()[:2] == b'#!')
   def normalized(info, mode=0o755 if executable else 0o644):
    info.mode=mode; info.uid=0; info.gid=0; info.uname='root'; info.gname='root'; return info
   tar.add(p,arcname=p.relative_to(stage),recursive=False,filter=normalized)
