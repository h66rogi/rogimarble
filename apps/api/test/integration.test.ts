...
test('next-turn travel reserves exactly one turn and supports selection, pause, resume and cancellation',async()=>{
  ...
  let task=await arrive();response=await send('choose_destination',{taskId:task.id,cellId:destination,expectedTaskRevision:task.revision,moveNow:true});assert.equal(response.status,201);
  let immediate=(await response.json() as any).result;assert.equal(immediate.travelStatus,'moved');assert.equal(immediate.toCellId,destination);assert.equal(immediate.reservedTurnCommandId!==null,true);
  await refresh();assert.equal(state.session.currentCellId,destination);assert.equal(state.effectTasks.some((x:any)=>x.id===task.id&&x.status==='pending'),false);
  task=await arrive();response=await send('choose_destination',{taskId:task.id,cellId:destination,expectedTaskRevision:task.revision});assert.equal(response.status,201);assert.equal((await response.json() as any).result.travelStatus,'waiting');
