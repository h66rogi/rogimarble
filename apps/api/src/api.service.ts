  ...
  async command(operator:Operator,channelId:string,sessionId:string,body:SessionCommandRequest):Promise<SessionCommandDto>{
    ...
    return transaction(async client=>{
      ...
      if(body.type==='roll_dice'){
        ...
        const effects=body.payload.triggerArrivalEffects?await executeCellEffects(client,session,session.board_definition,body.payload.cellId,'land',body.commandId,operator.id,afterCommands,{index:0,movements:0}):[];
...
export type EffectContext={index:number;movements:number;travelCreated?:boolean};
async function executeMovementEffects(client:pg.PoolClient,session:SessionRow,board:BoardDefinition,path:readonly string[],commandId:string,operatorId:string|null,after:(()=>Promise<void>)[],ctx:EffectContext={index:0,movements:0}){
...
export async function executeCellEffects(client:pg.PoolClient,session:SessionRow,board:BoardDefinition,cellId:string,trigger:'pass'|'land',commandId:string,operatorId:string|null,after:(()=>Promise<void>)[],ctx:EffectContext):Promise<any[]>{
  ...
  for(const effect of effects){if(++ctx.index>128)throw new UnprocessableEntityException('Effect chain exceeds safety limit');const index=ctx.index;let result:unknown={};
    ...
    else if(effect.type==='counter_settle'){
      const counter=(await client.query<any>('SELECT value FROM session_counters WHERE session_id=$1 AND counter_id=$2 FOR UPDATE',[session.id,effect.counterId])).rows[0];
      if(!counter)throw new UnprocessableEntityException('Counter missing');
      const reserved=Number((await client.query<any>(`SELECT COALESCE(sum(settlement_amount),0) value FROM missions WHERE session_id=$1 AND settlement_counter_id=$2 AND settlement_on='mission_completion' AND status='pending'`,[session.id,effect.counterId])).rows[0].value);
      const amount=counter.value-reserved;
      if(amount>0){
        const id=randomUUID(),createdAt=new Date();
        await client.query('UPDATE session_counters SET value=value-$3,revision=revision+1,updated_at=now() WHERE session_id=$1 AND counter_id=$2',[session.id,effect.counterId,amount]);
        after.push(()=>client.query(`INSERT INTO missions(id,session_id,message,quantity,status,shield_item_id,shield_quantity,created_by_command_id,source_effect_index,settlement_counter_id,settlement_amount,settlement_on,created_at) VALUES($1,$2,$3,$4,'pending',$5,$6,$7,$8,$9,$4,'creation',$10)`,[id,session.id,effect.message,amount,effect.shield?.itemId??null,effect.shield?.quantity??null,commandId,index,effect.counterId,createdAt]).then(()=>undefined));
        result={missionId:id,counterId:effect.counterId,quantity:amount,settleOn:'creation'};
      }else result={counterId:effect.counterId,quantity:0};
    }
...
async function applyTravelDecision(client:pg.PoolClient,session:SessionRow,task:TravelRow,decision:ReturnType<typeof decideTravel>,commandId:string,operatorId:string|null,after:(()=>Promise<void>)[]):Promise<unknown>{
  ...
  const effects=decision.reservation.onArrival==='trigger'?await executeCellEffects(client,session,session.board_definition!,decision.cellId,'land',commandId,operatorId,after,{index:0,movements:0}):[];
