...
export const api = {
  ...
  currentPawnAppearance:()=>request<PawnAppearanceDto>(operatorApi.pawnImage(channelId()),{cache:'no-store'}),
  selectPawnStyle:(styleId:PawnStyleId,expectedRevision:number)=>request<PawnAppearanceDto>(operatorApi.pawnStyle(channelId()),{method:'PUT',body:JSON.stringify({styleId,expectedRevision})}),
