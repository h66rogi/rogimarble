...
import type { RollResultDto } from '@rogimarble/contracts';
import { buildRollTimeline, rollPlayback } from '@/integrated-overlay/roll-playback';
...
export function useRollPresentation(input:{sessionKey:string|null;presentationEpoch:number|null;authoritativeCellId:string;boardPath:readonly string[]}) {
  ...
  const play=useCallback((event:{commandId:string;sessionKey:string;presentationEpoch:number;result:RollResultDto})=>{
    ...
    const parsed=rollPlayback({commandId:event.commandId,sessionId:'presentation',sessionEpoch:0,presentationEpoch:event.presentationEpoch,type:'roll_dice',afterRevision:0,result:event.result,createdAt:''},input.boardPath,event.result.toCellId);
    ...
    if(!parsed.dice.length){setView(current=>({...current,cellId:event.result.toCellId,dice:[],effectPhase:'idle',moving:false,trailCellIds:[]}));return true;}
    if(reducedMotion){setView(current=>({...current,cellId:event.result.toCellId,dice:parsed.dice,effectPhase:'idle',moving:false,trailCellIds:[]}));return true;}
