...
export interface Song {
  ...
  isFavorite?: boolean;
  /**
   * 다중 악보 슬롯. 매니저 권한일 때만 응답에 포함.
   * sortOrder 오름차순 정렬됨. 빈 배열 = 매니저인데 슬롯 없음. 키 자체가 없음 = 비매니저.
   */
  sheetMusics?: SheetMusicSlot[];
  /**
   * @deprecated 단일 슬롯 응답과 호환하기 위한 첫 슬롯 URL.
   */
  sheetMusicUrl?: string | null;
  /**
   * @deprecated 단일 슬롯 응답과 호환하기 위한 첫 슬롯 타입.
   */
  sheetMusicType?: SheetMusicType | null;
}

/**
 * 단일 악보 슬롯. backend SheetMusicSlotResponse 와 동일.
 */
...
/**
 * 악보 업로드/삭제는 별도 channel-scoped endpoint
