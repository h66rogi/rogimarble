...
const resultNames: Record<string, string> = { matched: '규칙 일치', no_match: '일치하는 규칙 없음', failed: '처리 실패', pending: '처리 대기', held:'운영자 확인 필요', ignored:'게임 미적용' };
...
export function MarbleDataPanel({ view }: { view: 'donations' | 'operations' | 'obs' | 'config' | 'sessions' | 'missions' }) {
  ...
  const load = async (append = false, background = false) => {
    ...
    if (!background) { busyRef.current = true; setBusy(true); setMessage(''); }
    try {
      if (view === 'donations') {
        const page = await api.donations({ ...(filters.donor ? { donor: filters.donor } : {}), ...(filters.result ? { result: filters.result } : {}), ...(append && cursor ? { cursor } : {}) });
        if (requestId !== sequence.current) return;
        setItems(previous => append ? appendUnique(previous, page.items) : background && hasAdditionalPages.current ? mergeUnique(page.items, previous) : page.items);
        if (append) hasAdditionalPages.current = true;
        if (!background || !hasAdditionalPages.current) setCursor(page.nextCursor);
    ...
    } catch (cause) { if (!background && requestId === sequence.current) setMessage(cause instanceof Error ? cause.message : '데이터를 불러오지 못했습니다.'); }
    finally {
      if (background) backgroundBusyRef.current = false;
      else if (requestId === sequence.current) { busyRef.current = false; setBusy(false); }
    }
  };
  useEffect(() => {
    setItems([]); setCursor(null); setIssued(null); hasAdditionalPages.current = false;
    void load();
    const timer = view === 'donations' ? window.setInterval(() => void load(false, true), 4000) : null;
    return () => { if (timer != null) window.clearInterval(timer); sequence.current += 1; busyRef.current = false; backgroundBusyRef.current = false; };
  ...
  if (view === 'donations') return <Panel title="후원 내역" description="후원자·개수·메시지와 처리 결과를 확인합니다.">
    {!connected && <p className="rounded-md border p-3 text-sm text-muted-foreground">수집 연결을 확인하고 있습니다. 저장된 후원 내역은 계속 확인할 수 있습니다.</p>}
    <form className="flex flex-wrap gap-2" onSubmit={event => {event.preventDefault(); const next={ donor: donor.trim(), result }; if (next.donor === filters.donor && next.result === filters.result) void load(); else setFilters(next);}}><Input className="min-w-0 flex-1" aria-label="후원자 검색" placeholder="후원자 이름" value={donor} onChange={event => setDonor(event.target.value)} /><select className="h-9 rounded-md border bg-background px-2 text-sm" aria-label="처리 결과" value={result} onChange={event => setResult(event.target.value)}><option value="">전체 결과</option>{Object.entries(resultNames).map(([value,name]) => <option key={value} value={value}>{name}</option>)}</select><Button disabled={busy}>조회</Button></form>
    {(items as readonly DonationEventDto[]).map(item => <article className="space-y-1 rounded-md border p-3 text-sm" key={item.id}><div className="flex flex-wrap items-center gap-3"><strong className="flex-1">{item.donorDisplayName}</strong><strong>{item.amount}개</strong><span>{resultNames[item.result] ?? item.result}</span></div>{item.message && <p className="break-words">{item.message}</p>}{donationReason(item.resultDetail) && <p className="text-sm text-muted-foreground">{donationReason(item.resultDetail)}</p>}<p className="text-xs text-muted-foreground">{formatDate(item.occurredAt)}{item.ruleId ? ` · 규칙 ${item.ruleId}` : ''}</p>{item.resultDetail != null && <details className="text-xs text-muted-foreground"><summary>처리 상세</summary><pre className="overflow-auto">{typeof item.resultDetail === "string" ? item.resultDetail : JSON.stringify(item.resultDetail, null, 2)}</pre></details>}</article>)}
  ...
  if (view === 'operations') return <Panel title="운영 기록" description="수동 조작과 설정 변경 내역입니다."><Button variant="outline" disabled={busy} onClick={() => void load()}>새로고침</Button>{items.map((item,index) => { const record=item as Record<string,unknown>; return <article className="space-y-2 rounded-md border p-3 text-sm" key={String(record.id ?? index)}><div className="flex flex-wrap items-center gap-2"><strong className="flex-1">{operationNames[String(record.operation_type)] ?? String(record.operation_type ?? '운영 조작')}</strong><span className="rounded-full bg-muted px-2 py-1 text-xs">{operationSource(record)}</span><time className="text-xs text-muted-foreground">{formatDate(String(record.createdAt ?? ''))}</time></div>{typeof record.reason === 'string' && <p>{reasonNames[record.reason] ?? record.reason}</p>}<details><summary className="cursor-pointer text-xs text-muted-foreground">변경 전·후 상세</summary><pre className="mt-2 max-h-64 overflow-auto rounded bg-muted p-2 text-xs">{JSON.stringify({before:record.before_state,after:record.after_state},null,2)}</pre></details></article>; })}{!busy && !items.length && <p className="text-sm text-muted-foreground">표시할 운영 기록이 없습니다.</p>}{cursor && <Button variant="outline" disabled={busy} onClick={() => void load(true)}>더 보기</Button>}{feedback}</Panel>;
...
function mergeUnique(fresh: readonly unknown[], previous: readonly unknown[]) {
  const ids = new Set(fresh.map(item => String((item as { id?: unknown }).id)));
  return [...fresh, ...previous.filter(item => !ids.has(String((item as { id?: unknown }).id)))];
}
function appendUnique(previous: readonly unknown[], next: readonly unknown[]) {
  const ids = new Set(previous.map(item => String((item as { id?: unknown }).id)));
  return [...previous, ...next.filter(item => !ids.has(String((item as { id?: unknown }).id)))];
}
function donationReason(detail: unknown) {
  const reason = typeof detail === 'string' ? detail : detail && typeof detail === 'object' && 'reason' in detail ? String((detail as { reason: unknown }).reason) : '';
...
function operationSource(record: Record<string, unknown>) {
  ...
  if (String(record.operation_type ?? '').startsWith('config.')) return '설정 관리';
  return '운영자 수동 조작';
}
