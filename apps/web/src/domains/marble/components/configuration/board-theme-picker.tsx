...
import { Board, BOARD_THEMES } from "@rogimarble/overlay-ui";
...
export function BoardThemePicker({ board, selected, change }: {
      ...
      {BOARD_THEMES.map(theme => <Card key={theme.id}>
        <CardContent className="space-y-3">
          <div className="pointer-events-none aspect-video overflow-hidden bg-muted" aria-hidden="true">
            <Board board={board} tokenCellId={board.startCellId} themeId={theme.id} fit reducedMotion />
          </div>
