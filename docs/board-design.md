...
운영자 화면의 세계여행 `이동`은 목적지 선택과 정상 차례 1회 예약을 하나의 명령으로 처리한다.
선택한 칸으로 즉시 이동하며, 이미 예약된 차례가 있으면 새 차례를 만들지 않고 그 차례를 소비한다.
이동 제한이 활성화된 동안에는 이 명령을 거부한다. 목적지만 미리 저장하는 기존 명령은 다음 차례 이동 규칙을 유지한다.
세계여행 예약이 남아 있는 동안 후원 이동은 대기열에서 굴리지 않는다. 스트리머가 목적지를 정해
`이동`하거나 수동 주사위로 여행 차례를 시작해야 다음 후원을 처리한다. 목적지 사전 입력만으로
대기 중인 후원 주사위를 자동 실행하지 않는다.
...
무인도 이동 제한이 남아 있는 동안 후원 주사위와 직접 이동은 모두 원래 순서로 대기한다.
탈출 판정은 스트리머의 수동 주사위 명령으로만 진행하고, 해제 후 아직 굴리지 않은 후원을 순서대로 재개한다.
...
무인도 중 486 등 직접 이동은 요청을 보존하고 제한 해제 뒤 처리한다. 수동 주사위 명령은
후원 대기열과 별도로 탈출 판정을 실행하므로 대기 후원을 소비하거나 순서를 바꾸지 않는다.
이 사유와 대기 순서를 콘솔에 표시한다.
...
초기·복원·표시 실패 시 기본 배치는 2026-09-25 `preview` 채널의 게시 레이아웃으로 고정한다.
보드·보유 아이템·후원 메뉴·채팅창을 표시하고, 1920×1080 투명 화면과 라벤더 드림 테마를 사용한다.
정확한 정규화 좌표·겹침 순서·채팅 표시 옵션은 `DEFAULT_MARBLE_OVERLAY_LAYOUT` 한 곳에서 관리한다.
