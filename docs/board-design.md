...
더블 탈출에 맞추어 이번 보드의 기본 주사위는 **6면 2개, 합산**으로 제안한다.
이전 문서의 1개 제안을 대체하며 개수·면 수는 `dice` 설정에서 변경할 수 있다.
1개로 바꾸려면 무인도 해제도 단순 횟수 제한이나 특정 눈으로 바꾸어야 하며 모순된 설정은 게시 검증에서 거부한다.
...
skip_rolls_or_doubles는 기본 주사위 두 개로 판정하며 실패 시 잔여 횟수 1을 차감한다.
...
counter_settle의 creation은 정산 미션 생성과 차감을 함께 기록하고, mission_completion은 생성 시
대상 수량을 예약한 뒤 완료/방어 시 확정한다. 이번 초기 설정은 **mission_completion**, 실드는 편집 가능한
제안값 **불허**다. 미션에는 counterId, quantity, unit, settlementId를 저장한다. 메시지에서 잔 수를 파싱하지 않는다.
총 3잔이면 3잔을 예약한 단일 미션을 만들고 완료 시 그 3잔만 차감한다. 대기 중 1잔이 추가되면 완료 후 1잔이 남는다.
실드를 허용하도록 설정하면 지정된 실드 비용으로 이 미션 전체를 방어하고 예약 수량도 정산한다.
미션 면제 역시 예약분을 정산하며, 정산 취소는 예약을 풀어 적립량에 남긴다. 둘은 별도 운영 작업이다.
예약을 제외한 수량이 0이면 청산은 기록만 남기고 빈 미션을 만들지 않는다. 동시/연속 청산은 같은 수량을
중복 예약하지 못한다. 이미 차감한 creation 정산을 취소할 때는 원본과 연결된 복원 원장을 남긴다.
...
무인도의 더블 조건에는 주사위 두 개가 필요하고 현재 칸 제외 후 후보가 없는 여행 설정도 게시를 거부한다.
