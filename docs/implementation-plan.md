...
- 인벤토리 증감/지정·원장, 미션 안내 기록, roll/direction/position/move_steps 명령. 미션별 완료·면제 확인은 운영 범위 밖이다.
...
- 청산 당시 적립 전량을 즉시 차감하고 미션 안내 한 건에 수량 snapshot을 남긴다. 이후 적립은 다음 청산까지 보존한다.
- 이전 보드의 mission_completion 설정도 실행 시 즉시 차감하고 기존 예약은 한 번 정산한다. ×2는 다음 실제 주사위 이동 한 회, repeat_roll 재귀 금지·적용 순서 고정.
- 무인도 3회 실패 뒤 다음 차례, 더블 합계 이동, 판정 중 modifier 보존/소모와 수동 제한 해제.
- 세계여행 차례 예약·선입력·원인 후원자의 선택권·만료/취소 시 같은 미실행 차례 반환.
- 목적지 barrier와 무인도 보류 직접 이동을 구분해, 뒤의 탈출 판정까지 막히는 교착 방지.
...
적립 증감/총량 지정·발생 즉시 청산, modifier 부여/수정/해제, lock 잔여/조건/해제, 여행 예약을 제공한다.
...
통과: 청산 효과의 한 번 차감과 미션 확인 없는 세션 종료, 위치 보정 후 대기/특수 상태 보존,
...
| 실드 정책·전체 보상·미션 안내 | M02, M06, M07, I02 | 보상 잔량 검증·미션별 확인 없이 기록 보존 |
