...
Application images are published to GHCR by the trusted GitHub Actions release workflow and consumed only by immutable digest. Terraform does not create a registry and grants no ECR permissions. New GHCR packages default to private; an organization owner must change each package to Public in GitHub package settings after its first publish, then verify an unauthenticated digest pull. EC2 stores no GitHub PAT, registry login, or credential file. PostgreSQL, Redis, and Caddy use reviewed upstream image digests.
