...
| 말 위치 | 홈 보드 칸 클릭 → 해당 칸의 `이 칸으로 이동` 버튼 | 공유 말의 실제 위치 보정, 도착 효과 실행, OBS 동기화 |
| 칸 수 이동 | N칸 이동, 현재 방향 또는 정/역방향 선택 | 경로를 따라 이동하고 지정한 칸 효과 정책 적용 |
| 미션 안내 | 발생한 미션을 게임 기록에서 읽기 전용으로 확인 | 개별 완료·면제·실드 사용 확인은 운영 화면 범위 밖 |
...
| 영역 | 내용 | 자주 쓰는 조작 |
| --- | --- | --- |
| 상단 고정 바 | 채널/세션, 수집/OBS 연결, 미처리 후원 수 | 상태 확인 |
| 중앙 보드 | 서버 위치, OBS 표시 중 위치, 방향 화살표, 칸 이름·선택 표시 | 칸 클릭 → 해당 칸의 위치 이동 버튼, 원하는 칸 요청 처리 |
| 우측 상단 즉시 액션 | 현재 턴에 가능한 굴리기·목적지 선택·이동 제한 조정 등 | 현재 필요한 조작만 표시 |
| 우측 하단 Pill Tabs | 적립/보상, 게임 조작, 게임 기록, 오버레이 | 적립·아이템 조정, 진행·판 효과·종료, 발생 내역 조회, 통합 OBS 주소 복사·실시간 배치 |
| 하단 실시간 패널 | 최신 채팅·후원, 검색/필터, 후원 처리 상태와 선택 행의 상세 | 방송 중 메시지와 후원 흐름을 한 화면에서 확인 |
| 대기 작업 탭 | 이동·목적지·여행 예약, 연차 진행도, 보류 사유 | 목적지 확정, 보류/취소 |
| 개발자도구의 운영 기록 | 수동 조작·설정 변경·실패·보정 이력 | 변경 전후 비교, 연결된 원본 보기 |

데스크톱에서는 보드·조작 패널·최근 후원을 함께 볼 수 있게 한다. 좁은 화면에서는 탭으로 전환하되
상태 바를 유지하고, 일시정지는 `게임 조작` 탭에서 제공한다. 후원 전체 내역은 별도 `/donations` 페이지에서도 조회할 수 있다.
즉시 액션과 하단 Pill Tabs는 간격·배경·경계로 별도 영역처럼 보이게 한다. Pill Tabs가 그 아래 전체 영역을 책임지며 독립된 조작 카드나 지난 항목을 탭 밖에 두지 않는다. 게임을 종료해도 `게임 기록`에서 직전 세션을 읽을 수 있다.
...
수량 덮어쓰기는 패널에서, 위치 보정은 클릭한 보드 칸의 이동 버튼으로 적용하고, 세션 종료·대기열 일괄 취소는
영향받는 요청 수를 보여준다. 게임 관리 명령의 사유는 화면에서 입력받지 않고 시스템이 기록한다.
...
실제 발생한 미션별 완료·면제·실드 사용 확인은 운영 범위에 포함하지 않는다. 과거 명령은 기록 호환을 위해 유지하지만 홈에서 개별 미션을 조작하지 않는다.

현재 세션의 아이템 카드를 설정 데이터에서 생성한다. 실드 외에 추가한 보상도 같은 화면·API를 사용한다.
카드에는 이름·현재 수량·마지막 변경 시각과 증감/직접 입력을 표시한다.
...
입력 중 후원 지급으로 재고가 바뀌면 revision 충돌을 표시하고 최신 수량으로 다시 확인한다.
새로고침·서버 재시작 후에도 수량이 유지된다. 게임 중 아이템 정의를 삭제해 잔량·미완료 미션을
고아 데이터로 만들지 않으며 필요하면 비활성화하고 참조를 보존한다.

### 4.1 술 적립·정산과 특수 상태

아이템 재고와 술 적립은 다른 원장으로 관리한다. 술 적립 카드에는 현재 총량을 표시한다.
청산 효과가 발생하면 당시 적립량을 즉시 차감하고 미션 안내를 기록한다.

| 대상 | 표시 | 조작과 서버 계약 |
| --- | --- | --- |
| 술 적립 | counter 이름/단위, 현재 총량, 증감 원인 | +/−/최종 총량 지정 |
| 술 청산 | 청산 당시 잔 수와 연결된 미션 안내 | 청산 효과가 발생하면 현재 전량을 즉시 차감; 미션별 완료 확인은 하지 않음 |
...
| 세계여행 예약 | 원인 도착·선택권자·목적지·차례 대기/입력 대기/실행 상태 | 판 배치에서 목적지를 골라 `이동`하면 그 명령이 정상 차례 1회를 소비하고 즉시 이동한다. 기존 목적지 선지정·취소도 유지 |

적립량 조정은 이미 생성한 정산 미션의 수량을 몰래 변경하지 않는다. 청산 뒤 새로 적립된 수량은
다음 청산까지 남고, 같은 명령을 재전송해도 추가 차감하지 않는다.

모든 상태 조작은 대상 revision·sessionEpoch·권한·사유와 원장/이력을 남긴다.
수동 위치 변경 패널에서 이동 제한 유지/해제를 명시하고 기본은 유지한다. 위치 변경만으로 배수·여행 예약·
적립/정산을 지우지 않는다. 미션 확인 없이 세션을 종료할 수 있으며, 미완료 여행 요청은 별도로 처리한다.
...
홈 보드 칸을 누르면 해당 칸의 보드 안쪽에 `이 칸으로 이동` 버튼을 표시한다. 같은 칸을 다시 누르거나
보드 밖을 누르거나 Escape를 누르면 버튼을 닫고, 다른 칸을 누르면 버튼이 그 칸으로 옮겨간다.
API는 화면 번호가 아닌 안정적인 cellId를 받는다.

홈에서 직접 보정하면 **즉시 위치 변경 + 자동 이동 일시정지 + 도착 칸 효과 실행**을 함께 확정한다.
서버가 일시정지와 위치 변경을 같은 잠금/트랜잭션으로 확정하여 다음 자동 이동이 끼어들지 않게 한다.
이미 커밋된 결과는 취소하지 않는다. 과거 칸 통과 보상·lap·실드·완료 미션도 자동으로 되돌리지 않는다.
도착 칸이 미션이면 이 보정에 연결된 새로운 미션을 한 번 생성한다.
...
| 새 세션 | 현재 게시된 보드 버전의 기본 방향/초기 위치·수량으로 시작; 이월은 명시적 선택과 기록 |
...
| 술 적립 3회→청산→추가 적립 1회 | 청산 시 수량 3인 안내 한 건과 적립량 0, 이후 1잔; 새로고침/재시도에도 동일 |
| 미션 안내가 남은 상태에서 세션 종료 | 개별 완료·면제 확인 없이 종료, 이전 미션 기록은 보존 |
...
- 보드 설정 → 방송 테마·배치: 6개 테마와 4개 방송 글꼴, 메뉴판 제목/항목/통화 단위,
  주사위 가격 규칙/직접 표시 값을 편집하고 게시한다.
- 홈 → 게임 관리 → 오버레이 및 상단 오버레이 설정 → 실시간 배치: 같은 통합 오버레이 편집기에서 메뉴판/가격 패널을 켜고 끌어 옮기거나
  크기를 조절한다. 움직이는 동안 150ms 단위 저장, 제스처 종료 시 마지막 값을 저장한다.
- 홈의 오버레이 탭에는 파츠별 방송 스타일을 표시하지 않는다. 파츠별 테마·글꼴은 상단 오버레이 설정에서 편집한다.
