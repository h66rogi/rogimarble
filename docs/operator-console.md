...
| 말 위치 | 홈 보드 칸 클릭 → 해당 칸의 작은 이동 버튼 | 공유 말의 실제 위치 보정, 도착 효과 실행, OBS 동기화 |
...
| 중앙 보드 | 서버 위치, OBS 표시 중 위치, 방향 화살표, 칸 이름·선택 표시 | 칸 클릭 → 해당 칸의 위치 이동 버튼과 효과 선택, 원하는 칸 요청 처리 |
...
홈 보드 칸을 누르면 해당 칸 가까이에 `이 칸으로 이동` 버튼만 표시한다. 같은 칸을 다시 누르거나
