...
| 오버레이 설정·복구 | 통합·파츠별 읽기 전용 URL 발급/재조회/회수, 권장 크기·스타일·위치, 연결 확인, 연출 건너뛰기·재동기화 |
| 기록·진단 | 원인 후원 → 규칙 버전 → 결과 → 운영 보정 확인, 작업자/전후 값/사유, 가능한 보정 되돌리기 |

오버레이 주소는 멜로밍의 채널 오버레이 토큰 조회 흐름처럼 로그인한 콘솔에서 다시 확인한다.
기존에 해시만 저장한 주소는 원문을 복원할 수 없으므로 새 주소를 발급해 OBS에서 교체한다.
