...
실드는 공유 세션 인벤토리에 적립하고, 운영자가 미완료 미션에서 사용한다.
...
| 보상·미션 | 실드 등 보상 추가/차감/수량 지정, 아이템 정의·지급량·실드 정책, 변경 이력, 미션 완료/면제/방어 |
...
| OBS 설정·복구 | 읽기 전용 URL 발급/회수, 연결 확인, 화면 크기·배경·위치, 연출 건너뛰기·재동기화 |
| 기록·진단 | 원인 후원 → 규칙 버전 → 결과 → 운영 보정 확인, 작업자/전후 값/사유, 가능한 보정 되돌리기 |

운영 화면은 상단 세션/연결 상태, 중앙 보드, 우측 조작·보상 패널, 하단 후원 목록과 대기/운영 기록 탭으로
구성한다. 보드와 조작 패널을 보면서 최근 후원을 확인할 수 있게 한다. 전체 내역은 별도 검색 페이지도 제공한다.
...
초기 인증은 배포 시 일회성 CLI로 생성한 실제 관리자 계정과 서버 세션으로 제공한다.
기본 공용 비밀번호를 배포하지 않는다. 관리자·운영자·조회 권한과 채널 소유권을 구분하고,
쿠키 보안·CSRF·로그인 제한을 적용한다. SOOP 계정 연동 로그인은 추후 rogichat 연계 기능이다.
첫 채널 연결은 배포 관리자의 소유권 확인을 거친다. 임의 채널 전체 수집을 허용하지 않는다.

서비스 관리자 화면은 `/admin`으로 별도 제공한다. 계정·채널·권한·로기챗 계정 연결과 감사 기록을
관리하며, 공유 로그인과 독립적인 관리자 아이디·비밀번호 및 별도 세션을 사용한다.
기본 공개 비밀번호나 자동 관리자 가입은 두지 않는다. [관리자 화면 계약](admin-console.md)과
[확인된 로그인 연동 상태](authentication.md)를 따른다.
...
- **말 위치:** 보드 칸/번호로 목표를 선택한다. 기본은 즉시 보정과 자동 이동 일시정지를 함께 확정하며
  도착 미션·통과 보상·lap를 자동 적용하지 않는다. 도착 미션 실행은 선택하고 N칸 이동은 별도 명령이다.
...
사용자 답변에 따라 술 적립은 도착마다 +1잔, 청산은 누적 전량을 한 번에 수행하는 단일 미션,
다음 턴 두 배는 다음 주사위 이동 거리 ×2를 1회 적용한다. 3잔 적립 상태에서는 수량 3인 청산 미션을
만들며 예약 후 새 적립분까지 지우지 않는다. 초기 청산 차감 시점은 미션 완료/방어/면제, 실드 불허로 제안한다.

무인도는 3회 쉬기 또는 더블 탈출, 세계여행은 다음 정상 차례를 원하는 칸으로의 직접 이동으로 대체한다.
더블 탈출에 맞춰 주사위는 6면 2개 합산을 새 초기 제안으로 두고 개수·면 수를 편집할 수 있게 한다.
1개로 변경 시 무인도 해제 조건도 호환되게 변경해야 한다. 일반 더블 추가 차례/여행 비용은 도입하지 않는다.
...
보드 효과/행·열/path 변경은 새 게시 버전과 새 세션부터 적용한다. 자세한 타입/실행/편집 계약은 보드 설계를 따른다.
...
미션 완료와 실드 사용은 같은 미션 상태/재고 트랜잭션을 경합한다. 이미 완료되었거나 방어된 미션은
재처리하지 않고 잔량 부족을 거부한다. 수동 보정도 idempotency key·사유가 있는 같은 명령 경로를 쓴다.
...
| 항목 | 현재 기준 |
| --- | --- |
| 보드 칸 수·배치·문구 | 전달 이미지의 9×6 외곽 26칸으로 시작하고 웹에서 편집; 무인도 더블에 맞춰 주사위 2개/6면을 초기 제안 |
