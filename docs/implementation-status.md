...
연속 후원은 수집 inbox에 남기고 한 번에 한 이동만 실행한다. 세계여행 예약 또는 무인도 제한이
생기면 후원 이동을 보류하고 운영자의 목적지 결정·수동 주사위 뒤에 재개한다. OBS는 최근 50개
이동 결과를 받아 재생 중인 경로가 끝난 뒤 다음 결과를 재생한다.

범위의 한계: SOOP 후원 자동 연결, 기한 만료 작업자, 모든 선택형 미션·추가 주사위 타입,
Lottie 자산 업로드/편집과 영속 PresentationCue·재연결 복구는 아직 남아 있다. OBS는 1초 상태 조회 방식이다.
