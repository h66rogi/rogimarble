...
export function Board({ board, tokenCellId, moving = false, dice, interactive = false, selectedCellId, onCellSelect }: {
  board: BoardDefinition; tokenCellId: string; moving?: boolean; dice?: readonly number[]; interactive?: boolean; selectedCellId?: string; onCellSelect?: (id: string) => void;
}) {
  const tokenCell = board.cells.find(c => c.id === tokenCellId) ?? board.cells[0];
  const tokenRect = getCellRect(board, tokenCell.id);

  return <div className="board-scroll" style={{ '--board-aspect': board.canvas.width / board.canvas.height } as CSSProperties}><span className="board-scroll-hint">좌우로 밀어 전체 {board.path.length}칸 보기</span><div className="board-stage" style={{ aspectRatio: `${board.canvas.width}/${board.canvas.height}` }}>
    <svg className="board-svg" viewBox={`0 0 ${board.canvas.width} ${board.canvas.height}`} role="img" aria-label={`${board.path.length}칸 주루마블 보드`}>
      <defs><filter id="cell-shadow"><feDropShadow dx="0" dy="4" stdDeviation="5" floodOpacity=".13" /></filter></defs>
      {board.cells.map(cell => { const r = getCellRect(board, cell.id); const round = cell.appearance.shape === 'circle' ? Math.min(r.width, r.height) / 2 : 15; return <g key={cell.id} className={`board-cell ${selectedCellId === cell.id ? 'selected' : ''}`} onClick={() => interactive && onCellSelect?.(cell.id)} onKeyDown={event => { if (interactive && (event.key === 'Enter' || event.key === ' ')) { event.preventDefault(); onCellSelect?.(cell.id); } }} role={interactive ? 'button' : undefined} tabIndex={interactive ? 0 : undefined}>
        <rect x={r.x} y={r.y} width={r.width} height={r.height} rx={round} fill={cell.appearance.fill} stroke={selectedCellId === cell.id ? '#5f3dc4' : cell.appearance.borderColor} strokeWidth={selectedCellId === cell.id ? 4 : 2} filter="url(#cell-shadow)" />
    ...
    <div className="token-wrapper" style={{ left: `${((tokenRect.x + tokenRect.width / 2) / board.canvas.width) * 100}%`, top: `${((tokenRect.y + tokenRect.height / 2) / board.canvas.height) * 100}%` }}><TokenLottie active={moving} /></div>
  </div></div>;
}
